...
public class TreeViewWindow {
    ...
    private VBox buildContent() {
        ...
        root.getChildren().add(sectionTitle(" Binary Search Tree "));
        root.getChildren().add(buildBSTSection());

        root.getChildren().add(sectionTitle("↩ Undo Stack  "));
        root.getChildren().add(buildStackSection(service.getUndoStack().toList(), C_UNDO));

        root.getChildren().add(sectionTitle("↪ Redo Stack "));
     ...
     */
    private void drawBSTNode(GraphicsContext gc, TaskBST.BSTNode node, double cx, double cy, double offset, int depth) {
        if (node == null) return;
